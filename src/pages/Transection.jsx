      ...
      <div className="px-6 pt-6 pb-10 relative">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="relative">
            <button
              className={`py-2 px-2 cursor-pointer border border-r-0 my-2 rounded-l-md ${forSearch.month_type == "m"
            ...
            <button
              className={`py-2 px-4 cursor-pointer border border-r-0 my-2 ${forSearch.month_type == "d"
                  ? "bg-gray-800 text-white"
                  : "bg-gray-200 text-black"
                }`}
            ...
            <button
              className={`py-2 px-4 cursor-pointer border my-2 rounded-r-md mr-2 ${forSearch.month_type == "y"
                  ? "bg-gray-800 text-white"
                  : "bg-gray-200 text-black"
                }`}
        ...
        <div className="h-[70vh] overflow-auto text-white">
            ...
            <tbody>
              {Transections.length > 0 ? (
                Transections.map((item) => (
                  <tr key={item.id} className="bg-gray-500 hover:bg-gray-700">
                    ...
                    <td className="border px-4 py-2 text-center">
                      {item.paymentMethod == "qrcode" ||
                        ...
                        item.paymentMethod == "qrcode" ? (
                          <i className="fa-solid fa-qrcode text-blue-800 mr-2"></i>
                        ) : (
                          <i className="fa-solid fa-wallet text-gray-100 mr-2"></i>
                        )
                      ) : (
                        item.paymentMethod
                      )}
          ...
          <div className="pt-8 absolute bottom-3">
            <span>ທັງໝົດ </span>
            <span className="text-amber-500">{TableObject.amount}</span>{" "}
            <span> ລາຍການ</span>
          </div>
        </div>
      </div>
